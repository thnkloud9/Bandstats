...
DashboardController.prototype.bandStatsAction = function(req, res) {
	...
	bad_facebook: function(cb) {
    	    parent.bandRepository.count({"running_stats.facebook_likes.error": { $exists : true }}, function(err, count) {
        	cb(err, count);
    	    }); 
	},
	bad_lastfm: function(cb) {
    	    parent.bandRepository.count({"running_stats.lastfm_listeners.error": { $exists : true }}, function(err, count) {
        	cb(err, count);
    	    }); 
	},
    },
